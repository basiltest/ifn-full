#!/usr/bin/env python3
"""
Mid-Internship Review deck for Basil Stevenson (Lumenor).
Generates a 6-slide 16:9 .pptx, institutional / "issued, not branded" look.
Image placeholders are positioned for screenshots Basil drops in via Impress.
...
def footer(s, n):
    ...
    text(s, MX, Inches(7.04), Inches(8), Inches(0.3),
         "Basil Stevenson  ·  Lumenor  ·  Mid-Internship Review",
         size=9, color=MUTED)
    text(s, Inches(9.9), Inches(7.04), Inches(2.49), Inches(0.3),
         "%02d / 06" % n, size=9, color=MUTED, align=PP_ALIGN.RIGHT)
...
# ===========================================================================
s = slide()
text(s, MX, Inches(1.35), CW, Inches(0.4),
     "SOFTWARE ENGINEERING INTERNSHIP", size=14, color=ACCENT, bold=True, letter=2.0)
text(s, MX, Inches(1.85), CW, Inches(1.4),
     "Mid-Internship Review", size=52, color=INK, bold=True)
text(s, MX, Inches(3.0), CW, Inches(0.5),
     "Lumenor   ·   May to July 2026", size=21, color=MUTED)
rect(s, MX, Inches(3.75), Inches(2.0), Pt(2.2), fill=ACCENT)
text(s, MX, Inches(4.0), Inches(10.8), Inches(0.8),
     "Three work-streams: QA on FlowX, a product landing page, and the IFN platform.",
     size=17, color=INK)
# identity block
rect(s, MX, Inches(5.55), CW, Pt(1.2), fill=HAIR)
text(s, MX, Inches(5.75), Inches(8), Inches(0.4),
     "Basil Stevenson", size=17, color=INK, bold=True)
text(s, MX, Inches(6.18), Inches(8), Inches(0.7),
     [[("ICFAI  ·  BCA (IP2)", {"color": MUTED, "size": 13})],
      [("Company tutor: Rama Raju sir", {"color": MUTED, "size": 13})]],
     space_after=2)
...
# ===========================================================================
s = slide()
y = header(s, "Context", "The company and my work")
text(s, MX, y, CW, Inches(0.9),
     [[("Lumenor", {"bold": True}),
       ...
       ("), plus ", {}),
       ("coreX", {"bold": True}),
       (" for internal teams. My first half centered on ", {}),
       ("FlowX", {"bold": True}),
       (": Inventory, Sales and CRM, HRMS, Accounting, Procurement.", {})]],
     size=15, color=INK, spacing=1.06)

cards = [
    ("01", "QA on FlowX", "Flow-tested FlowX: 67 tests, 19 bugs logged, including 2 security criticals."),
    ("02", "Lumenor landing page", "Built and shipped the public product marketing site."),
    ("03", "IFN platform", "Built ICFAI Founders Network, a members-only founder platform."),
]
cw = Inches(3.62); gap = Inches(0.30); cx = MX; cy = Inches(3.05); ch = Inches(2.5)
for num, t, d in cards:
    ...
    text(s, cx + Inches(0.3), cy + Inches(1.34), cw - Inches(0.6), Inches(1.0),
...
text(s, MX + Inches(0.25), Inches(5.97), CW, Inches(0.5),
     [[("Closing the loop: ", {"bold": True, "color": INK}),
       ("IFN is built for my own university's founder community.", {"color": INK})]],
     size=14)
footer(s, 2)
...
# ===========================================================================
s = slide()
y = header(s, "Work-stream 1  ·  Quality Assurance", "Testing FlowX")
...
# ===========================================================================
s = slide()
y = header(s, "Work-stream 2  ·  Frontend", "Lumenor landing page")
...
# ===========================================================================
s = slide()
y = header(s, "Work-stream 3  ·  Flagship build", "IFN: ICFAI Founders Network")
...
cx = MX; cy = Inches(2.5); row_max = Inches(12.4)
for lab in labels:
    w = Inches(0.42 + 0.095 * len(lab))
    if cx + w > row_max:
        cx = MX; cy = cy + Inches(0.6)
    chip(s, cx, cy, lab)
...
# two screenshots (top-anchored: keep nav + content, trim bottom)
...
# ===========================================================================
# SLIDE 6 - Next + learnings
# ===========================================================================
s = slide()
y = header(s, "Outlook", "What's next and what I learned")
colw = Inches(5.55)
# left
text(s, MX, Inches(2.15), colw, Inches(0.35),
     "Second half", size=14, color=ACCENT, bold=True)
...
text(s, rx, Inches(2.15), colw, Inches(0.35),
     "What I learned", size=14, color=ACCENT, bold=True)
...
rect(s, MX, Inches(5.95), Inches(0.09), Inches(0.5), fill=ACCENT)
text(s, MX + Inches(0.25), Inches(5.97), CW, Inches(0.5),
     "Halfway in: bugs found, products shipped, more to build.",
     size=15, color=INK, bold=True)
footer(s, 6)

# ---------------------------------------------------------------------------
out = "/home/basil/lumenor/ifn/presentation/Lumenor_Internship_MidReview.pptx"
prs.save(out)
